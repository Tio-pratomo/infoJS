...
app.get('/test-log', (req, res) => {
  ...
  logger.info({ userId: 123, status: 'success' }, 'Pengujian log dengan detail pengguna.');
...
1.  Kita akan menambahkan properti `enabled` ke konfigurasi `pino`.
...
2.  Simpan perubahan di `utils/logger.js`.
3.  Jalankan ulang aplikasi ( `node index.js`). Perhatikan output log.
4.  Sekarang, ubah nilai `LOG_ENABLE` di file `.env` menjadi `false`:
...
5. Simpan file `.env` dan jalankan ulang aplikasi. Anda seharusnya tidak melihat pesan log dari Pino di terminal. Ini membuktikan bahwa sakelar kita berfungsi!

6. Kembalikan `LOG_ENABLE` ke `true` agar kita bisa melanjutkan.
